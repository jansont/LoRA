...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, lora_config, scale=False):
...
class MLP(nn.Module):
    def __init__(self, n_state, config, lora_config):  # in MLP: n_state=3072 (4 * n_embd)
...
class Block(nn.Module):
    def __init__(self, n_ctx, config, lora_attn, lora_mlp, scale=False):
        super(Block, self).__init__()
        nx = config.n_embd
        self.ln_1 = LayerNorm(nx, eps=config.layer_norm_epsilon)
        
        self.attn = Attention(nx, n_ctx, config, lora_attn, scale)
        
        self.ln_2 = LayerNorm(nx, eps=config.layer_norm_epsilon)
        self.mlp = MLP(4 * nx, config, lora_mlp)
...
class GPT2Model(nn.Module):
    def __init__(self, config, lora_configs):
        ...
        for i in range(config.n_layer):
            attn_config, mlp_config = get_lora_config(i, lora_configs)
            blocks.append(Block(config.n_ctx,
                                config,
                                lora_attn=attn_config,
                                lora_mlp=mlp_config, 
                                scale=True))
...
class GPT2LMHead(nn.Module):
    def __init__(self, model_embeddings_weights, config):
        ...
        self.n_embd = config.n_embd
        self.weight = model_embeddings_weights
        self.set_embeddings_weights(model_embeddings_weights)

    def set_embeddings_weights(self, model_embeddings_weights):
        embed_shape = model_embeddings_weights.shape
        self.decoder = nn.Linear(embed_shape[1], embed_shape[0], bias=False)
        self.decoder.weight = self.weight  # Tied weights
...
class GPT2LMModel(nn.Module):
    def __init__(self, config, lora_configs):
        ...
        self.transformer = GPT2Model(config, lora_configs)
